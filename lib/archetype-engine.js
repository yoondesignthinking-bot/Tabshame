  ...
  function diagnose(tabRecords, opts = {}) {
    ...
    for (const archetype of archetypes) {
      ...
      if (allMatched) {
        ...
        // The primary rule's matched value is the canonical "how many
        // tabs trigger this persona" signal. Used as the primary sort
        // key below so 6 LinkedIn tabs beat 5 YouTube tabs (the user's
        // intuition: more tabs = stronger claim on the persona).
        const primaryCount = (ruleResults[0] && ruleResults[0].result.value) || 0;
        candidates.push({ archetype, vars, primaryCount });
      }
    }

    // Sort order:
    //   1. Primary-rule matched count DESC — "more tabs wins" override.
    //      Users with 6 LinkedIn tabs and 5 YouTube tabs now get
    //      The LinkedIn Lurker (6) even though YouTube Rabbit-Holer has
    //      a higher static priority.
    //   2. Priority DESC — breaks ties when counts are equal. This keeps
    //      rare/specific archetypes (Job Hunt Marathon, Pre-Date
    //      Detective) winning over generic ones when both fire on the
    //      same tab count.
    //   3. Declaration order — stable-sort fallback for total ties.
    candidates.sort((a, b) => {
      if (b.primaryCount !== a.primaryCount) {
        return b.primaryCount - a.primaryCount;
      }
      return (b.archetype.priority || 0) - (a.archetype.priority || 0);
    });
